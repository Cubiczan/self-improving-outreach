...
def test_pipeline_skips_auto_voice_when_flag_false(tmp_path: Path):
    lead = _cfo_lead()
    transcript = tmp_path / f"{lead.lead_id}.json"
    transcript.write_text(
        json.dumps({"lead_id": lead.lead_id, "positive": True, "notes": "should not apply"}),
        encoding="utf-8",
    )
    settings = Settings(
        mock_mode=True,
